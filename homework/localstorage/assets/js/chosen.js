let createzone=document.querySelector('.chosen')
let carsChosen=JSON.parse(localStorage.getItem('card'))
...
function displayChosen(){
    ...
    <div id="${car.id}" class="card" style="width: 18rem;">
        ...
        <div class="card-body">
        <h5 class="card-title"><b>${car.model}</b></h5>
        <p class="card-text">${car.info}</p>
        <p class="card-address">${car.address}</p>
        <a href="#" class="btn btn-primary">${car.learn}</a>
        </div>
        </div>
        `
...
function chechCars(counterClick){
        ...
        if (counterClick===1) {
         let createrow =document.createElement('div')
          ...
          createrow.id=++id
          createzone.append(createrow)
          return createrow
        }else{
          let createrow =document.querySelector('.customrow')
          return createrow
        }
}